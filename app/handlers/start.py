...
async def require_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if username not in settings.allowed_usernames:
        ...
        return None
    return db.upsert_user(tg_user.id, username, settings.display_names[username])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    display_name = existing_user["display_name"] if existing_user else settings.display_names[username]
