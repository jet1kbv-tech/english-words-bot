...
- `➕ Создать урок` — учитель выбирает ученика из allowed student users, вводит обязательный `title`, затем опциональные `theme` и `grammar_topic` (`-`/`skip`/`пропустить` означает пустое значение). После этого создаётся строка `lessons` со статусом `draft` и показывается summary созданного урока.
- `📋 Мои уроки` — показывает последние 10 уроков текущего teacher user с title, student, theme и status.
...
`ADMIN` получает обычное student menu с дополнительной кнопкой `🛠 Админ`; `TEACHER` получает отдельное teacher menu с разделом `📚 Уроки`. Тренировки, игра на 10 слов, ошибки и обмен словами остаются прежними и работают через `require_user()`. Все остальные получают отказ при `/start` или при попытке пройти `require_user()`.
