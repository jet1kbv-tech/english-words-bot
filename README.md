...
- 🤖 AI-помощник (внутри урока в статусе `DRAFT`) — кнопка `✨ Создать контент с AI` запускает генерацию **черновика** содержимого урока (AI Draft Generator v1, см. ниже).

Teacher role не добавляет admin-функций и не меняет game flow.

### AI Draft Generator v1 (черновик содержимого урока)

Внутри урока в статусе `DRAFT` раздел `🤖 AI-помощник` позволяет сгенерировать **черновик** слов, грамматических карточек и упражнений с помощью AI. Это первый шаг к будущему AI Lesson Builder: AI не создаёт урок напрямую, не пишет в таблицы урока и не публикует/назначает контент ученику — черновик существует только в памяти Telegram-сессии (`context.user_data`) до тех пор, пока учитель не закроет диалог или не перезапустится бот.

Шаги мастера после `✨ Создать контент с AI`:

1. **Тема** — свободный текст (3–200 символов).
2. **Уровень** — A1 / A2 / B1 / B2 / C1.
3. **Слова** — 0 / 5 / 10 / 15 / 20 (по умолчанию рекомендуется 10).
4. **Грамматика** — 0 / 1 / 2 / 3 (по умолчанию рекомендуется 2).
5. **Упражнения** — 0 / 3 / 5 / 7 / 10 (по умолчанию рекомендуется 5).
6. **Подтверждение** — экран со сводкой параметров и кнопками `✨ Сгенерировать`, `← Изменить`, `Отмена`.

Все параметры проверяются на уровне сервиса (а не только в Telegram callback), включая суммарный лимит `слова + грамматика + упражнения ≤ 30` и требование хотя бы одного ненулевого счётчика. Если в уроке уже есть контент, перед генерацией показывается предупреждение о том, что AI создаст отдельный черновик и не изменит существующие материалы.

После успешной генерации показывается сводка `✨ Черновик готов` с кнопками `👀 Посмотреть` (постраничный просмотр: слова — по 5 на страницу, грамматика и упражнения — по одной карточке на страницу, с показом правильного варианта ответа), `✅ Сохранить в урок`, `🔄 Сгенерировать заново` (повторная генерация с теми же параметрами; прошлый черновик остаётся доступным, если новая генерация завершится ошибкой) и `Отмена`. На один урок в один момент времени допускается только одна активная генерация; ошибки AI-сервиса и невалидного ответа показываются безопасным сообщением без технических деталей.

`✅ Сохранить в урок` атомарно переносит слова (с переводом и примером), грамматические карточки и упражнения (с вариантами, правильным индексом и объяснением) из черновика в те же таблицы урока, что и ручное добавление контента, — и они читаются теми же существующими Runtime/repository методами. Сама запись, однако, выполняется отдельной атомарной операцией (`LessonService.save_generated_draft` → `LessonRepository` → одна SQLite-транзакция), а не теми же поштучными add-методами, которыми пользуется ручное добавление, — так гарантируется «всё или ничего». Если хотя бы один из непустых в черновике разделов (слова/грамматика/упражнения) уже содержит материалы, сохранение полностью отклоняется с предупреждением, ничего не перезаписывая и не объединяя. Повторное сохранение одного и того же черновика (двойное нажатие, повтор Telegram-обновления, перезапуск бота) не создаёт дублей — это защищено на уровне SQLite (`lessons.ai_draft_generation_id`), а не только состоянием Telegram-сессии. После успешного сохранения показываются реальные сохранённые количества и черновик удаляется из временного состояния; ученик, которому назначен урок, проходит сохранённый контент обычным runtime-flow без каких-либо изменений в нём.

Пока черновик не сохранён, он существует только в памяти Telegram-сессии (`context.user_data`) и теряется при закрытии диалога или перезапуске бота — это ожидаемое ограничение. Объединение с уже непустыми разделами урока не реализовано.

### Teacher Draft Editor (редактирование слов черновика)

На экране `✨ Черновик готов` кнопка `✏️ Редактировать` открывает редактор черновика: сводку `Слова: N / Грамматика: N / Упражнения: N` и кнопки `📝 Слова — N` / `📚 Грамматика — N` / `🧩 Упражнения — N` / `👀 Посмотреть весь черновик` / `✅ Сохранить в урок`. В этом этапе редактируется только раздел **слова** — кнопки грамматики и упражнений показывают toast «появится в одном из следующих обновлений» вместо перехода на несуществующий экран.

Раздел слов — постраничный список (`N. word — перевод`, по 5 на страницу) с карточкой на каждое слово (`✏️ Изменить слово` / `✏️ Изменить перевод` / `✏️ Изменить пример` / `🗑 Удалить` / `⬅️ К списку`) и кнопкой `➕ Добавить слово`. Правка и добавление запрашивают у учителя текст следующим сообщением (с кнопкой `Отмена`) и проверяют его по тем же правилам, что и AI-генератор (непустое слово и перевод, лимиты длины, уникальность слова без учёта регистра, слово ≠ перевод; пример необязателен). Удаление требует явного подтверждения. Все изменения применяются только к `GeneratedLessonDraft` в памяти для этого урока — ничего не пишется в SQLite до `✅ Сохранить в урок`, которая сохраняет уже отредактированный черновик тем же существующим save-flow (см. выше); ручные правки не создают новый `generation_id`. Устаревшие или потерянные черновики (например, после перезапуска бота или после того, как черновик уже сохранён) показывают безопасное сообщение «Этот черновик больше недоступен или был обновлён. Откройте урок и перейдите к актуальному черновику.» вместо ошибки. Редактирование грамматики, упражнений, изменение порядка слов и история изменений не реализованы.
