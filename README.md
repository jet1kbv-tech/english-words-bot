...
После успешной генерации показывается сводка `✨ Черновик готов` с кнопками `👀 Посмотреть` (постраничный просмотр: слова — по 5 на страницу, грамматика и упражнения — по одной карточке на страницу, с показом правильного варианта ответа), `🔄 Сгенерировать заново` (повторная генерация с теми же параметрами; прошлый черновик остаётся доступным, если новая генерация завершится ошибкой) и `Отмена`. На один урок в один момент времени допускается только одна активная генерация; ошибки AI-сервиса и невалидного ответа показываются безопасным сообщением без технических деталей.

Черновик **не сохраняется в SQLite** и не создаёт слова/грамматику/упражнения в уроке — сохранение и объединение с существующим контентом запланированы в отдельном будущем PR.
