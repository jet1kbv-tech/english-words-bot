...
- Короткая игровая сессия `🎮 Игра на 10 слов`: 10 карточек из личного словаря с миксом новых, слабых и небольшого числа сильных слов; после `✅ Знаю` / `❌ Не знаю` бот показывает ответ и в конце выдаёт итог.
...
В игровом режиме доступны кнопки: `👀 Показать ответ`, `✅ Знаю`, `❌ Не знаю`, `⏭ Пропустить`, `🛑 Закончить`; `🛑 Закончить` досрочно завершает сессию и показывает текущий итог.
